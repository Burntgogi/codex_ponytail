...
- Codex CLI `0.145.0`
- Ponytail `4.8.4`
- 고정 커밋 `16f29800fd2681bdf24f3eb4ccffe38be3baec6b`
...
1. 업스트림 파일은 수정하지 않는다.
2. 브랜치나 태그 대신 검토한 전체 커밋 SHA에 고정한다.
...
> 중요: 위 커밋에 포함된 업스트림 `.agents/plugins/marketplace.json`은 플러그인 소스를 다시 `ref: main`으로 지정한다. 따라서 `codex plugin marketplace add DietrichGebert/ponytail --ref <SHA>`만 사용하는 방식은 플러그인 소스까지 고정한다는 보장이 없다. 아래처럼 로컬 marketplace manifest에서 플러그인 URL의 `ref`를 직접 SHA로 고정한다.
...
- 이미 `ponytail` 항목이 있지만 URL이나 `ref`가 다를 때
- 검토하지 않은 사용자 변경과 편집 범위가 겹칠 때

## 2. personal marketplace 준비
...
Codex의 `apply_patch`로 `$manifest` 위치에 다음 파일을 생성한다.
...
두 해시가 같은지 확인한 다음, `apply_patch`로 기존 `plugins` 배열에 아래 객체 하나만 추가한다. 기존 항목을 재정렬하거나 다시 직렬화하지 않는다.
...
예상 상태는 `ponytail@personal`, `installed, enabled`, 버전 `4.8.4`다.
...
- [ ] 설치된 버전은 `4.8.4`, Git HEAD는 고정 SHA다.
...
이 설치는 고정 포인터 방식이므로 업스트림 새 버전을 자동으로 따라가지 않는다. 업데이트할 때는 다음 순서를 새 작업으로 수행한다.
...
검토 없이 `main`으로 전환하거나 자동 업데이트를 켜지 않는다.
